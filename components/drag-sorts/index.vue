	...
	<view class="">
		<movable-area class="drag-sort" :style="{height:boxHeight }" id="drag">
			<movable-view v-for="(item, index) in currentList" :key="index" :x="item.x" v-if="item.isShow === 1"
				:data-index="index" @touchstart="touchstart" @touchmove.stop="touchmove" @touchend="touchend"
				:y="item.y" :direction="direction" disabled damping="40" :animation="item.animation"
				class="drag-sort-item" :style="styleObject"
				:class="{'active': active == index, 'vh-1px-t': item.index > 0}" @click="close(index)">
				<view class="item">
					<view class="text">
						<view class="icon-bg flex flexmid" :style="{background:item.bgColor}">
							<text class="iconfont flex1" :class="item[props.text]"></text>
						</view>
					</view>
					<text class="iconopt">
						<text class="iconfont" :class="item[props.text2]"></text>
					</text>
					<text class="label-text">{{item[props.label]}}</text>
				</view>
			</movable-view>
		</movable-area>
	</view>
	...
	export default {
		...
		methods: {
				...
				for (const key in list) {
					...
					if (key <= 3) {
						...
						if ((Number(key) + 1) % 4 === 0) {
							...
							x = ((Number(key) + 1) % 4 - 1) * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this
								.windowWidth * 0.04
						}
			...
			moveUpdateCurrentList(index) {
				for (const i in this.currentList) {
					...
					if (index == key) {
						...
						if (key <= 3) {
							this.currentList[i].x = key * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this
								.windowWidth * 0.04
						} else {
							if ((Number(key) + 1) % 4 === 0) {
								this.currentList[i].x = 3 * this.windowWidth * 0.24 + this.windowWidth * 0.04 || this
									.windowWidth * 0.04
							} else {
								this.currentList[i].x = ((Number(key) + 1) % 4 - 1) * this.windowWidth * 0.24 + this
									.windowWidth * 0.04 || this.windowWidth * 0.04
							}
						}
					}
				}
				...
				this.currentList.every((res, index) => {
					...
					if (0 < absX && absX <= 10 && absY > 0 && absY <= 10 && this.active != index) {
						...
						this.currentList.every((_res, _index) => {
							// 判断从大像小移还是从小向大移
							if (this.currentList[this.active].SortNumber < this.currentList[index]
								.SortNumber) {
								// 移动元素比目标元素所在位置小，之间元素排序--
								if (this.currentList[_index].SortNumber > this.currentList[this.active]
									.SortNumber && this.currentList[_index].SortNumber <= this.currentList[
										index].SortNumber) {
									_res.SortNumber--
								}
							} else {
			...
			close(index) {
				...
				var data = [this.currentList, this.currentList[index]]
