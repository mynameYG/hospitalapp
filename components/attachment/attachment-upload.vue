	...
	<view>
		<view class="atts-item"  v-for="(item,index) in fileList" :key="index">
			...
			<view class="flex" v-if="item.fileType == 'image' || matchType(item.fileName) == 'image'" >
				<view class="atts-image" @click='previewImg(item)' >
			...
			<!-- 文件 -->
			<view class="flex" v-else  @click="previewOther(item.downloadUrl)">
		...
		methods: {
			...
			previewImg() {
				...
				this.fileList.forEach(item =>{
					// console.log(item);
					// console.log(this.matchType(item.fileName))
					if(this.matchType(item.fileName) == 'image'){
