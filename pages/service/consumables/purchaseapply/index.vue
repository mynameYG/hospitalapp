<template>
	<view>
		<pulldownRefresh ref="pulldownRefresh" @refresh="pulldownRefresh">
			<scroll-view class="panel-scroll-box" scroll-y @scrolltolower="loadMoreData('add')">
				<view class="out mb15" v-for="item of list" @click="goto(item.id)">
					...
					<image v-if="item.status == 'pass'" src="/static/img/yinzhang/yitongguo.png"></image>
					<image v-else-if="item.status == 'fail'" src="/static/img/yinzhang/weitongguo.png"></image>
					<image v-else src="/static/img/yinzhang/daishenhe.png" mode=""></image>
					<view class="btn_repair mt10"  v-if="item.status != 'out'">
						<view class="btn-del"  v-if="resourcesCode.includes('purchase-del')" @click.stop="del(item.id)"> 删除</view>
						<view class="btn-sh" v-if="resourcesCode.includes('purchase-edit') && item.status != 'pass'"  @click.stop="edit(item.id)">编辑</view>
						<view class="btn-bj" v-if="resourcesCode.includes('purchase-check') && item.status == 'report'" @click.stop="check(item.id)">审核</view>
					</view>
				</view>
				<!-- 打补丁 -->
		...
		<view class="xinzeng" v-if="resourcesCode.includes('purchase-add')">
			<view class="repair_btn" @click="add()">新增采购</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods:{
			check(id) {  //挂起
				uni.navigateTo({
					url: './purchaseapply-audit?id='+id
				})
			},
			
		// 	auditcheck(result) {
		// 		this.$http.put(`/mobile/material/purchase/audit/${this.id}?auditReason=${this.proposal}&auditResult=${result}`).then(res =>{
		// 			this.pulldownRefresh();
		// 			this.proposal = "";
		// 		})
		// 		this.show = false;
		// 	},
			...
			edit(id) {
				uni.navigateTo({
					url: `./purchaseapply-editadd?id=${id}&type=edit`
				})
			},
			
			goto(id) {
				uni.navigateTo({
					url: './purchaseapply-details?id='+id
				})
			},
			
			add() {
				uni.navigateTo({
					url: './purchaseapply-editadd?type=add'
				})
			},
			...
			del(id){
				...
				uni.showModal({
					...
					success: function (res) {
						if (res.confirm) {
							self.$http.del(`/mobile/material/purchase/${id}`).then(res =>{
			...
			loadMoreData(type) {
				...
				let urlJson = `/mobile/material/purchase?page=${this.page}&pageSize=${this.pageSize}`;
