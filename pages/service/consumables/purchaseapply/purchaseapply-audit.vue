			...
			<template class="readmore" slot="readMore">
				...
				<view class="out mb15" v-for="(item,index) of info.details" :key="index">
					...
					<view class="flex mb15">
						...
						<view class="ml5">{{item.purchaseNumber}}{{item.unit}}</view>
					</view>
					<view class="flex mb15">
...
<script>
	...
	export default {
		...
		methods:{
			...
			submit(){
				this.$http.put(`/mobile/material/purchase/audit/${this.id}?auditReason=${this.proposal}&auditResult=${this.result}`).then(res =>{
					uni.navigateBack();
				})
			}
		}
