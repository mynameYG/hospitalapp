	...
	<view>
		<view class="out mb15" style="padding-top: 20rpx;">
			<view class="flex flexbet mb25 require">
				<view class="out_font">出库单号 <text class="ml5">:</text></view>
			...
			<view class="flex flexbet mb25 require" @click="$refs.manageUser.choose()" >
				<view class="out_font">接收人<text class="ml5">:</text></view>
				<person-picker ref="manageUser" type="manageUser" :multi="false" :persons.sync="manageUser"></person-picker>
				<template v-if="manageUser.length < 1">
		...
		<view class="out mb15" v-for="(item,index) of ruleForm.stocks" :key="index">
			...
			<view class="flex flexbet mb25 require">
				...
				<view class="ml5"><input type="text" v-model="item.unitPrice" placeholder="请输入"></view>
			</view>
			<view class="flex flexbet require">
				<view class="out_font">出库数量 <text class="ml5">:</text></view>
				<view class="ml5"><input type="text" v-model="item.outNumber" placeholder="请输入"></view>
			</view>
		</view>
		...
		onLoad(options) {
			if(options.type){
				...
				uni.setNavigationBarTitle({
					title: options.type == 'add' ? '新增出库' : '编辑'
				})
			}
			if(options.id){
				this.id = options.id
		...
		mounted() {
			...
			if(this.type == 'edit' && this.id) {
				this.$http.get(`/mobile/material/manage/out/${this.id}`).then(res =>{
					this.manageUser.push({
						name: res.handoverName,
						id: res.handover
					});
					this.ruleForm.outName = res.outName;
					this.ruleForm.outDate = res.outDate;
		...
		methods:{
			...
			add(){
				...
				var params = {
					...
					handover: this.manageUser[0].id,
					detail: this.ruleForm.stocks,
					files: files,
				}
				if(sign) {
					if(this.type == "add"){
						this.$http.post(`/mobile/material/manage/out/add`,params).then(res =>{
							uni.navigateBack();
						})
					}	
					
					if(this.type == "edit") {
						this.$http.put(`/mobile/material/manage/out/record/${this.id}`,params).then(res =>{
							uni.navigateBack();
						})
					}
				}
