<template>
	<view class="bg">
		<view class="inventory">
						...
						<radio-group @change="radioChange">
							<label class="uni-list-cell uni-list-cell-pd" v-for="(name, index) in item.list" >
								<view class="radio_list">
									<radio style="transform:scale(0.6)" color="#0DD474" :value="name.optionValue + '-' + item.id +'-'+ name.optionName" :checked="item.paramValue == name.optionValue" /><text>{{name.optionName}}</text>
								</view>
							</label>
						</radio-group>
					</view>
			...
			<template v-else>
				<view v-for="(item,index) in list" :key="index">
					<view class="shenhe" v-if="item.type.value == 'radio'">
						<view class="panduan">{{item.paramName}}</view>
						<radio-group @change="radioChange">
							<label class="uni-list-cell uni-list-cell-pd" v-for="(name, index) in item.list" >
								<view class="radio_list">
									<radio style="transform:scale(0.6)" color="#0DD474" :value="name.optionValue + '-' + item.id +'-'+ name.optionName" /><text>{{name.optionName}}</text>
								</view>
							</label>
						</radio-group>
					</view>
					
					<view class="shenhe" v-else-if="item.type.value == 'textarea' ">
						<view class="beizhu_title">{{item.paramName}}</view>
						<u--textarea  height="100" v-model="item.value" class="mb15" placeholder="请输入内容" count  confirmType="done"></u--textarea>
					</view>
					
					<view class="shenhe flex flexbet"  v-else-if="item.type.value == 'input'">
						<text class="beizhu_title">{{item.paramName}}</text>
						<input type="text" v-model="item.value">
					</view>
				</view>
		...
		<view class="bottom_btn">
			...
			<view  :class="{obsolete : sign}" class="submit" @click="submit()">提交</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			this.state = JSON.parse(options.state);
			this.sign = JSON.parse(options.sign);
			this.id = options.id; 
		...
		methods:{
			...
			//设备参数
			parameter(){
				this.$http.get(`/mobile/inspection/equipment/parameter/${this.id}`).then(res => {
					this.list = res;
				})
			},
			...
			// 获取回显内容
			getdetail(){
				this.$http.get(`/mobile/inspection/equipment/detail?equipmentId=${this.id}`).then(res => {
					console.log(res);
					if(this.state) {
						if(res.attachment){
			...
			submit() {
					...
					this.list1.forEach((item,index)=>{
						...
						if(item.type == "radio") {
							item.list.forEach(res=>{
								if(item.paramValue == res.optionValue) {
									item.aaa = res.optionName;
								}
							})
						}
				...
				if(this.sign) {
					this.$http.post(`/mobile/inspection/submit?equipmentId=${this.id}`,{
