			...
			<template>
				<view v-for="(item,index) in list" :key="index">
						...
						<radio-group @change="radioChange($event,list)">
							<label class="uni-list-cell uni-list-cell-pd" v-for="(name, index) in item.list">
								<view class="radio_list">
									<radio style="transform:scale(0.6)" color="#0DD474"
										:value="name.optionValue + '-' + item.id +'-'+ name.optionName +'-'+ name.id" />
									<text>{{name.optionName}}</text>
								</view>
							</label>
						</radio-group>
					</view>
					...
					<view class="shenhe" v-else-if="item.type.value == 'textarea' ">
						...
						<u--textarea height="100" v-model="item.value" class="mb15" placeholder="请输入内容" count
			...
			<view class="model-item atts-item" v-for="(item, index) in fileList">
				<view class="flex flexbet">
					<view class="atts-image">
						<image v-if="matchType(item.filePath) == 'image'" class="is-image"
							:src="fileRUrl(item.filePath)" :data-src="fileRUrl(item.filePath)"
			...
			submit() {
				...
				this.list.forEach((item, index) => {
					...
					params.push({
						...
						optionId: optionId,
						type: item.paramValue == "other" ? item.paramValue : item.type.value
					})
				})
				//对表单进行验证
