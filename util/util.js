...
const util = {
	...
	jumpWebPage:function(type){
		uni.navigateTo({
			url: `/PProperty/pages/service/components/webPage?type=${type}`
		});
	},
	...
	getWxCode(){
		...
		let promise = new Promise((resolve, reject) => {
			//获取本地存储
			let vinfo = uni.getStorageSync('vinfo');
				        ...
				        if (~res.provider.indexOf('weixin')) {
				            uni.login({
				                ...
				                success: function (loginRes) {
									...
									_self.$http.get(`/mobile/vx/openid?jscode=${loginRes.code}`).then(res =>{
										console.log('-----jscoderes',res)
										resolve({
											code: res
										});
										uni.setStorageSync('vinfo', res);
									})
				                }
				            });
				        }
	...
	openFileManager(file, domRef, type) {
		// TODO 苹果手机目前只支持上传图片,不支持其他格式文件
		if (plus.os.name.toLowerCase() != "android") {
			if (type && type != 'image') {
				uni.showToast({
					title: 'ios平台暂不支持' + typeJson[type] + '格式',
					icon: 'none'
				});
			} else {
				uni.chooseImage({
